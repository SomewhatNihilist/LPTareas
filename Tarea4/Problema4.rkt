...
; Argumento lista >= largo que lista_rep (los excesos de lista son ignorados)
(define (replicar lista lista_rep)
  (if (not (>= (length lista) (length lista_rep))) null
      (let aux ((ls lista) (lsr lista_rep) (outp (list)))
        (if (null? lsr) (reverse outp)
            (if (<= (car lsr) 0)
                (aux (cdr ls) (cdr lsr) outp)
                (aux ls (list* (- (car lsr) 1) (cdr lsr)) (list* (car ls) outp)))))))
...
(define test1 (replicar '(a b c d e) '(1 2 3))) ; Elementos de lista en exceso son ignorados
(define test2 (replicar '(a b c) '(1 2 3 4))) ; Retorna null cuando no se cumple
